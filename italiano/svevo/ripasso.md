...
```mermaid
flowchart LR
    A["Prefazione"] --> B["Dottor S.<br/>vendetta"]
    B --> C["verità + bugie"]
    A --> D["narratore inaffidabile"]
    E["Preambolo"] --> F["autoanalisi fallita"]
    F --> G["ritenterò domani"]
```
...
```mermaid
flowchart TD
    A["Inetto"] --> B["Ipertrofia della coscienza"]
    B --> C["Non agisce"]
    C --> D["Contemplatore"]
    E["Lottatore"] --> F["Agisce e afferra"]
    G["Zeno"] --> H["U.S. / padre / Augusta / Guido"]
    H --> I["Narratore inaffidabile"]
    I --> L["Verità e bugie"]
    G --> M["Finale"]
    M --> N["Vita = malattia"]
    N --> O["Ordigno"]
    O --> P["Nebulosa: nichilismo o palingenesi"]
```
