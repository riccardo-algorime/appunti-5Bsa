...
```mermaid
flowchart LR
    A["Trieste"] --> B["Città di confine"]
    A --> C["Rifugio"]
    A --> D["Gabbia"]
    B --> E["Culture italiana, slava,<br/>ebraica, austriaca"]
    C --> F["Cantuccio<br/>vita pensosa e schiva"]
    D --> G["Limite<br/>costrizione"]
    A --> H["Scontrosa grazia"]
```
...
```mermaid
flowchart TD
    A["Poesia onesta"] --> B["Chiarezza"]
    A --> C["Sincerità"]
    A --> D["Parole comuni"]
    A --> E["Verità interiore"]
    D --> F["Tradizione:<br/>fiore/amore, sonetto,<br/>titolo Canzoniere"]
    E --> G["Modernità:<br/>inconscio, trauma,<br/>ambivalenza"]
```
...
```mermaid
flowchart TD
    A["Città vecchia"] --> B["Porto, vie oscure,<br/>osterie, lupanari"]
    B --> C["Detrito di merci e uomini"]
    C --> D["Prostituta, marinaio,<br/>vecchio, dragone, giovane"]
    D --> E["Umanità marginale"]
    E --> F["Infinito nell'umiltà"]
    F --> G["Puro nel turpe"]
```
