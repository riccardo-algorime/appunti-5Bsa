...
```mermaid
flowchart TD
    A[Poetica in negativo] --> B[Nessuna certezza]
    A --> C[Nessuna formula]
    A --> D[Parola insufficiente]
    A --> E[Resistenza al dolore]
    E --> F[Male di vivere]
    B --> G[Relativismo novecentesco]
    G --> H[Pirandello]
```
...
```mermaid
flowchart TD
    A[Montale] --> B[Ossi di seppia 1925]
    B --> B1[Gobetti]
    B --> B2[Antifascismo]
    B --> B3[Rottami / scarto / residualità]

    A --> C[Poetica in negativo]
    C --> C1[Nessuna certezza]
    C --> C2[Nessuna formula]
    C --> C3[Resistenza al male di vivere]

    A --> D[Paesaggio ligure]
    D --> D1[Monterosso]
    D --> D2[Aridità]
    D --> D3[Disarmonia uomo-natura]

    A --> E[Immagini]
    E --> E1[Campana di vetro]
    E --> E2[Muraglia e cocci aguzzi]
    E --> E3[Varco]
    E --> E4[Correlativo oggettivo]

    A --> F[Testi analizzati]
    F --> F1[I limoni]
