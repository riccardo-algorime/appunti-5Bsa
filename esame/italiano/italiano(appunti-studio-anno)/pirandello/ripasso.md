...
```mermaid
flowchart LR
    A["Vecchia signora imbellettata"] --> B["Sembra ridicola"]
    B --> C["Avvertimento del contrario"]
    C --> D["Comico"]
    D --> E["Riflessione: forse soffre"]
...
```mermaid
flowchart TD
    A["Credo di essere UNO"] --> B["Gli altri mi vedono in modi diversi"]
    B --> C["Divento TANTI"]
    C --> D["Se rifiuto ogni forma divento NESSUNO"]
```
...
```mermaid
flowchart TD
    A["Mattia Pascal a Miragno"] --> B["Trappole familiari ed economiche"]
    B --> C["Monte Carlo: vincita casuale"]
    C --> D["Lo credono morto"]
    D --> E["Diventa Adriano Meis"]
    E --> F["Roma: Anselmo Paleari"]
    F --> G["Lanterninosofia"]
    E --> H["Senza documenti non può vivere"]
    H --> I["Finto suicidio"]
    I --> L["Ritorno a Miragno"]
    L --> M["Il fu Mattia Pascal"]
```
