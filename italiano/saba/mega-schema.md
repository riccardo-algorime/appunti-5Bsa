...
flowchart LR
    A["Trieste"] --> B["Rifugio"]
    A --> C["Gabbia"]
    B --> D["Cantuccio personale<br/>vita quotidiana<br/>amore per la città"]
    C --> E["Confine<br/>limite<br/>impossibilità di oltrepassare"]
    A --> F["Scontrosa grazia"]
```
...
```mermaid
flowchart LR
    A["Quello che resta<br/>da fare ai poeti"] --> B["Poesia onesta"]
    B --> C["Non artificiosa"]
    B --> D["Non costruita per stupire"]
    B --> E["Sentimenti genuini"]
    C --> F["Manzoni degli Inni sacri:<br/>mediocre ma autentico"]
    D --> G["D'Annunzio delle Laudi:<br/>sublime ma effimero"]
    E --> H["Pascoli:<br/>umili cose e quotidiano"]
```
...
```mermaid
flowchart TD
    A["Vita di Saba"] --> B["Esperienze private"]
    A --> C["Storia del Novecento"]
    B --> D["Infanzia · padre · madre · Peppa"]
    B --> E["Trieste · Lina · Linuccia"]
    C --> F["Fascismo · leggi razziali · guerra"]
    D --> G["Il Canzoniere"]
    E --> G
    F --> G
    G --> H["Autobiografia poetica"]
    G --> I["Storia e cronistoria del Canzoniere"]
```
...
```mermaid
flowchart LR
    A["Capra concreta<br/>sola e bagnata"] --> B["Belato"]
    B --> C["Dolore animale"]
    C --> D["Riconoscimento del poeta"]
    D --> E["Dolore universale"]
    E --> F["Poesia onesta:<br/>semplice ma profonda"]
```
...
```mermaid
flowchart TD
    A["Città vecchia<br/>porto, vicoli, lupanare"] --> B["Detrito di merci e uomini"]
    B --> C["Prostituta, marinaio,<br/>vecchio, dragone, giovane"]
    C --> D["Creature della vita e del dolore"]
    D --> E["Infinito nell'umiltà"]
    E --> F["Puro nel turpe"]
...
mindmap
  root((Umberto Saba))
    Vita
      ...
      Balia Peppa Sabaz
      Lina e Linuccia
      Libreria antiquaria dal 1919
      Leggi razziali
      Depressione e ricoveri
    Poetica
      Poesia onesta
      Chiarezza
      Sincerità
      Semplicità apparente
      Linguaggio discorsivo e narrativo
